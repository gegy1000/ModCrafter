...
public class GuiModCrafter extends GuiScreen
{
    ...
    private GuiModList modsGui;

    private GuiButton loadMod;
    ...
    public GuiModCrafter(GuiMainMenu mainMenu)
    {
    ...
    public void actionPerformed(GuiButton button)
    {
        ...
        {
            this.loadMod.enabled = false;
            this.modsGui.actionPerformed(button);
        }
    }

    public void loadMod(int modIndex)
    {
        if (!this.isModLoaded)
        {
            this.isModLoaded = true;

            Mod loadedMod = mods.get(modIndex);

            if (loadedMod.getName() == null)
            {
                loadedMod.rename("Mod" + modIndex + 1);
            }

            this.mc.displayGuiScreen(new GuiModCrafterProject(this, loadedMod));
        }
    }
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_)
    {
        ...
        this.modsGui.drawScreen(p_73863_1_, p_73863_2_, p_73863_3_);
    ...
    class GuiModList
    {
        ...
        public GuiModList()
        {
            this(GuiModCrafter.this.mc, GuiModCrafter.this.width, GuiModCrafter.this.height, 32, GuiModCrafter.this.height - 64, 36);
        }

        public GuiModList(Minecraft p_i1052_1_, int p_i1052_2_, int p_i1052_3_, int p_i1052_4_, int p_i1052_5_, int p_i1052_6_)
        {
            this.mc = p_i1052_1_;
        ...
        {
            return GuiModCrafter.this.mods.size();
        }

        /**
         * The element in the slot that was clicked, boolean for whether it was double clicked or not
         */
        protected void elementClicked(int selected, boolean doubleClicked, int p_148144_3_, int p_148144_4_)
        {
            GuiModCrafter.this.selectedMod = selected;
            boolean hasMod = GuiModCrafter.this.selectedMod >= 0 && GuiModCrafter.this.selectedMod < this.getSize();
            GuiModCrafter.this.loadMod.enabled = hasMod;

            if (doubleClicked && hasMod)
            {
                GuiModCrafter.this.loadMod(selected);
            }
        }
         ...
         */
        protected boolean isSelected(int modIndex)
        {
            return modIndex == GuiModCrafter.this.selectedMod;
        }
        ...
        {
            return GuiModCrafter.this.mods.size() * 36;
        }

        protected void drawBackground()
        {
            GuiModCrafter.this.drawDefaultBackground();
        }

        protected void drawSlot(int modIndex, int p_148126_2_, int p_148126_3_, int p_148126_4_, Tessellator p_148126_5_, int p_148126_6_, int p_148126_7_)
        {
            ...
            GuiModCrafter.this.drawString(GuiModCrafter.this.fontRendererObj, name, p_148126_2_ + 2, p_148126_3_ + 1, 0xFFFFFF); // GuiSelectWorld
            GuiModCrafter.this.drawString(GuiModCrafter.this.fontRendererObj, mod.getFileName() + " (Last Modified: " + dateFormat.format(new Date(mod.getLastModified())) + ")", p_148126_2_ + 2, p_148126_3_ + 10, 0x636363);
        }
        ...
        {
            Tessellator tessellator = Tessellator.instance;
            this.mc.getTextureManager().bindTexture(GuiModCrafter.background);
            GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
            float f = 32.0F;
            tessellator.startDrawingQuads();
    ...
    // public ModList()
    // {
    // super(GuiModCrafter.this.mc, GuiModCrafter.this.width, GuiModCrafter.this.height, 32, GuiModCrafter.this.height - 64, 36);
    // }
    //
    // protected int getSize()
    // {
    ...
    // protected void drawSlot(int modIndex, int p_148126_2_, int p_148126_3_, int p_148126_4_, Tessellator p_148126_5_, int p_148126_6_, int p_148126_7_)
    // {
    // String name = mods.get(modIndex).getName();
    //
    // if (name == null)
    // {
    // name = "Mod" + (modIndex + 1);
    // }
    //
    // GuiModCrafter.this.drawString(GuiModCrafter.this.fontRendererObj, name, p_148126_2_ + 2, p_148126_3_ + 1, 16777215);
    // }
    //
    // protected void drawContainerBackground(Tessellator tessellator)
    // {
    // GuiModCrafter.this.mc.getTextureManager().bindTexture(Gui.optionsBackground);
    // GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
    // float f1 = 32.0F;
    // tessellator.startDrawingQuads();
    // tessellator.setColorOpaque_I(2105376);
    // tessellator.addVertexWithUV((double)this.left, (double)this.bottom, 0.0D, (double)((float)this.left / f1), (double)((float)(this.bottom + this.getAmountScrolled()) / f1));
    // tessellator.addVertexWithUV((double)this.right, (double)this.bottom, 0.0D, (double)((float)this.right / f1), (double)((float)(this.bottom + this.getAmountScrolled()) / f1));
    // tessellator.addVertexWithUV((double)this.right, (double)this.top, 0.0D, (double)((float)this.right / f1), (double)((float)(this.top + this.getAmountScrolled()) / f1));
    // tessellator.addVertexWithUV((double)this.left, (double)this.top, 0.0D, (double)((float)this.left / f1), (double)((float)(this.top + this.getAmountScrolled()) / f1));
    // tessellator.draw();
    // }
    // }
}
